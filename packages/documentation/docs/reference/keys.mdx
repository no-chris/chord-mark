...
However, this is done as a best effort and might not always work as expected, especially if the song contains multiple keys.
So as a best practice, it is recommended to always explicitly declare a key.
 
The accidental are also properly chosen if the song contains multiple keys:
