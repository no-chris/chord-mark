...
There are a few - rare - cases where <CM/> might interpret a lyric line as a chord line.
When this occurs, you can force a line to be detected as lyric by adding the `:` symbol at the begining.

<TabbedChordMark src={'Ah Ah Ah\n:Ah Ah Ah\nFish\n:Fish'} />

Note that you could also add any other character to the line to force the lyric detection.
The only difference is that the `:` will be hidden at rendering time.
