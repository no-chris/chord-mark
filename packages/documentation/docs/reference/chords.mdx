...
Under the hood, <CM/> uses the powerful <CS/> library, which is virtually capable of parsing any chord symbol
and perform the normalization based on [widely recognized practices](https://github.com/no-chris/chord-symbol#background-information). 
