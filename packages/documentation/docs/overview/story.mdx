import { CM, CCS, CS } from '../../src/components/shorcuts';

# The story

## Why a new format?

Like many things in this world, <CM/> was born out of frustration, and my frustration was oriented towards existing chord charts.
...
However, there is also a non-trivial number of times when I find them incredibly frustrating:
...
-   **Transposing** them doesn't always work, because most software/website fails to recognize complex chords. You end up with a chord chart mixing tonalities that you need to fix manually.
-   They are **only useful for the singer musician**: if all that you care about is the song structure or the chord grid, as a bass player or drummer, for example, you will have a hard time getting that information in an accurate way from those chord charts.
...
I came up with my own way of writing them - that I used to write in a Word document - manually aligning chords and bar across lines in order to encode the rhythm structure of the songs.
While solving the issue about rhythm information, it only made the writing process worse, while not allowing for easy key adjustments.

## Enters ChordMark

I slowly came up with the idea of a new chord charts format that would solve all the issues mentioned above.

The first step was to design a new syntax, with the purpose of encoding the maximum of information in the easiest possible way, so complex song transcriptions could be quickly written with a minimum of efforts.
...
That is the purpose of the <CM /> Javascript/NodeJs module. 
...
I've been playing with the idea of <CM /> in my mind for years: looking into my archive, I found some design documents dated as far back as February 2015!

It took me 4 more years to actually start the development at the beginning of 2019, taking advantage of a professional break. 
...
some basic functionalities - such as aligning the chords over the lyrics - were still lacking, not to mention user documentation which remained non-existent for a very long time.

Nevertheless, I've been using the 2019 version of <CM/> for more two years for all my transcription work, and I have been so happy with it that I've finally decided to resume the development during the summer of 2021.
I've started implementing the features I missed during my 2 years of "beta-testing", and they turned out so great - in my opinion, at least! - that I feel the project is now mature enough to go public.
