...
- **ChordPro**: use this settings if the input source is in the [ChordPro](https://www.chordpro.org/) format, eg with chords encoded between square brackets directly in the lyrics
...
If <CCS/> will do the heavy conversion work for you, you will still need to finalize the chord chart by hand.
Especially, since <CM/> is the only format to encode rhythm information, this information needs to be added to each chord symbol.

There are 3 possibilities, depending on the duration of a chord:
...
Use the chord duration marker `.` to specify for how many beats the chord should be played.
...
You need to repeat the chord for as many times as it should be played for a full bar.
