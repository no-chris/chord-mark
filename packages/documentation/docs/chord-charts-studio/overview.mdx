...
`Chord Charts Studio` is the official editor of <CM />: it is freely available [online](http://chords-chart-studio.netlify.app).
...
The main user interface can be broken down into 4 parts:
...
To switch from one song to another, simply click on a title in the song list below the actions buttons.
...
Of course, the chord chart needs to be written following [the ChordMark syntax](/docs/getting-started).
- **Screen view**: this mode is useful if you wish to play the song by directly looking at your computer/tablet screen.
You can format the chord chart so it nicely fits the screen real estate and you don't need to scroll when playing (for example using multiple columns).
To further increase the screen real estate, you can fold both the right and left sidebars.
- **Print/PDF Preview**: this mode shows you a preview on how the song will look like when printed on an A4 paper or exported as a PDF. 
Here again, you can tweak the settings so the song fits on a single sheet of paper (or as many as you want): number of columns, font size, etc.
...
In other words, you can have a setting of 4 columns for the screen view and 2 columns for the printed one, for example.
The "Key" settings, however, are set once for all for all the modes.
...
- **Preferred accidentals**: only available if the previous option is turned on
    - **Auto**: the accidental with the most occurences in the song will be used 
...
- **Export format**: 
    - **ChordMark**: format the song using the ChordMark rendering
    - **ChordMark (Source)**: use this setting to export the song in the source ChordMark format
...
- **Auto repeat chords**: automatically repeat chords for identical section (verse, chorus...).
...
- **Chords color**: change the colors of the chords symbols when displayed on screen
- **Highlight chords**: further highlight the chords symbols
