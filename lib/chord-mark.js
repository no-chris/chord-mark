...
function chordLyrics_space(chordLineInput, lyricsLineInput) {
  ...
  var spacedChordLine = forEachChordInChordLine(chordLine, function (chord, chordIndex, barIndex) {
...
var tpl_lyricLine = __webpack_require__(7503);
...
function renderLyricLine_render(lyricLine) {
  return lyricLine_default()({
    lyricLine: lyricLine.model.lyrics
  });
}
...
function renderSong(parsedSong) {
  ...
  var song = allLines.filter(shouldRenderLine).map(function (line, lineIndex, allFilteredLines) {
    var rendered;
...
function shouldAlignChords(alignChordsWithLyrics, nextLine) {
  return alignChordsWithLyrics && nextLine && nextLine.type === lineTypes.LYRIC;
}
